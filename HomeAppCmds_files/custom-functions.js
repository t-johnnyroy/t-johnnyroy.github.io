...
function toggleCommandButton(buttonId, enabled) {
	// var commandButtonID = document.getElementById('commandButtonID').value;
	var commandbuttonID = buttonId;
	OfficeRuntime.ui.getRibbon().then(function (ret) {
		var dr = ret;
		var btn = {id: commandButtonID, enabled: enabled};
		var tab = {id: "OfficeAppTab1", controls: [btn]};
		var data = {tabs: [tab]};
...
CustomFunctions.associate('ToggleButton', function(buttonId, enabled) {
	return toggleCommandButton(buttonId, enabled);
});